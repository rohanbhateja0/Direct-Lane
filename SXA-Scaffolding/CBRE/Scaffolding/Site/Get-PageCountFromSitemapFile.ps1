<#
.SYNOPSIS
    Gets the total number of pages from all sitemap files in the media library and exports to Excel.

.DESCRIPTION
    This script retrieves all sitemap files from the media library path, filters for files with "-Live" in the name,
...
.EXAMPLE
    ...
    .\Get-PageCountFromSitemapFile.ps1

.EXAMPLE
    # Use SitemapPath parameter to specify a different path
    .\Get-PageCountFromSitemapFile.ps1 -SitemapPath "/sitecore/media library/Project/CBRE/Shared-Site/Emerald-Sitemaps"
#>
...
begin {
	Write-Verbose "Cmdlet Get-PageCountFromSitemapFile - Begin"
	...
	function Export-PageCountsToExcel {
		...
		try {
			...
			$baseFileName = "Sitemap_PageCounts_$timestamp"
			...
			if ($useImportExcel) {
				...
				$exportData | Export-Excel -Path $serverFilePath -WorksheetName "Sitemap Page Counts" -AutoSize -FreezeTopRow -BoldTopRow
...
process {
	Write-Verbose "Cmdlet Get-PageCountFromSitemapFile - Process"
	...
	Write-Host "`nRetrieving sitemap files from: $targetPath" -ForegroundColor Yellow
	...
	# Filter for items with "-Live" in the name
	$liveSitemapItems = $allItems | Where-Object { $_.Name -like "*-Live*" }
	
	if ($null -eq $liveSitemapItems -or $liveSitemapItems.Count -eq 0) {
		Write-Host "`nNo sitemap files found with '-Live' in the name." -ForegroundColor Yellow
		return
	}
	
	Write-Host "`nFound $($liveSitemapItems.Count) sitemap file(s) with '-Live' in the name." -ForegroundColor Green
	...
	foreach ($fileItem in $liveSitemapItems) {
		try {
			...
			if ($fileItem -is [Sitecore.Data.Items.MediaItem]) {
				...
				try {
					...
					Write-Host "[$index/$($liveSitemapItems.Count)] Skipping $($fileItem.Name) - Not a media item" -ForegroundColor Yellow
					$index++
					continue
				}
			}
			
			Write-Host "[$index/$($liveSitemapItems.Count)] Processing: $($mediaItem.Name)" -ForegroundColor Cyan
			...
			Write-Host "[$index/$($liveSitemapItems.Count)] Error processing $($fileItem.Name): $($_.Exception.Message)" -ForegroundColor Red
			$sitemapPageCounts += [PSCustomObject]@{
	...
	Write-Host "  Total Sitemap Files Processed: $($sitemapPageCounts.Count)" -ForegroundColor Green
	...
	if ($excelPath) {
		Write-Host "`nSitemap page counts exported to Excel successfully!" -ForegroundColor Green
	}
	...
	Write-Verbose "Cmdlet Get-PageCountFromSitemapFile - End"
}
